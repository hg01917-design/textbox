...
from .accounts import ensure_chrome_for, login_hint
...
def _extract_count_near_keywords(page, keywords: list[str]) -> int | None:
    ...
    for keyword in keywords:
        # "오늘 방문자수 123" / "오늘 방문자수: 123" / "오늘\n123" 등 다양한 배치 허용
        pattern = re.compile(re.escape(keyword) + r"[^0-9]{0,20}?([0-9][0-9,]*)")
...
def naver_today_views(blog_id: str, on_log=None) -> dict:
    ...
    try:
        ...
        page = get_page(browser, navigate_to=f"https://admin.blog.naver.com/{blog_id}")
        time.sleep(2)
        ...
        if _looks_like_login_url(page.url):
            ...
            return {
                "ok": False,
                "error": f"로그인이 필요합니다 — 포트 {port} Chrome 창에서 직접 로그인해주세요.{hint_text}",
            }

        views = _extract_count_near_keywords(page, ["오늘 조회수", "오늘조회수", "오늘 방문자수", "오늘방문자수"])
        if views is None:
            return {
                "ok": False,
...
def tistory_today_views(blog_id: str, on_log=None) -> dict:
    ...
    try:
        pw, browser = connect(port=port)
        page = get_page(browser, navigate_to=f"https://{blog_id}.tistory.com/manage/statistics/")
        time.sleep(2)
